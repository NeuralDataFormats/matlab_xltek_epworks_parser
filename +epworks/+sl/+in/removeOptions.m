...
if isstruct(varargin_data)
    was_struct = true;
    varargin_data = epworks.sl.in.structToPropValuePairs(varargin_data);
else
    was_struct = false;
end
...
if ~isempty(in.output_types)
    if in.output_types{1}(1) == 's'
       removed_options = epworks.sl.in.propValuePairsToStruct(removed_options); 
    end
    if in.output_types{2}(1) == 's'
       remaining_options = epworks.sl.in.propValuePairsToStruct(remaining_options);  
    end
