classdef history_dat_parser < handle
    %
    %   Class:
    ...
    methods
        function obj = history_dat_parser(file_path,logger)
            ...
            s_all(obj.n_entries) = epworks.parse.history.dat_entries;
            ...
            for iSeg = 1:obj.n_entries
                ...
                s.trace_id = data(cur_index:cur_index+15);

                s.trace = logger.getObjectByID(s.trace_id);
                ...
                s.IDs = ids1;
                temp = F.typeMatrix(temp(:,17:end),'uint64',false);
                s.timestamps = epworks.utils.processType3time(temp);
            end
            obj.entries = s_all;
